...
constexpr void compute(vector<string>& trees, const size_t r, const size_t c, char& highest, uint32_t& visible) {
	if(parse(trees[r][c]) <= highest)
		return;
	highest = parse(trees[r][c]);
	if(!seen_yet(trees[r][c])) {
		trees[r][c] += 10;
		++visible;
	}
}
...
int main(void) {
	...
	// For each col
	for(size_t c = 1; c < cols - 1; ++c) {
		char highest = trees[0][c]; // Can be viewed from top
		for(size_t r = 1; r < rows - 1; ++r)
			compute(trees, r, c, highest, visible);
