...
struct Path
{
	...
	{
		const auto [ r, c ] = pos;
		return direction == UP ? (dir != DOWN && r && grid[r - 1][c] != '#' && grid[r - 1][c] != 'v')
			: direction == DOWN ? (dir != UP && r != grid.size() - 1 && grid[r + 1][c] != '#' && grid[r + 1][c] != '^')
			: direction == LEFT ? (dir != RIGHT && c && grid[r][c - 1] != '#' && grid[r][c - 1] != '>')
			: (dir != LEFT && c != grid.front().size() - 1 && grid[r][c + 1] != '#' && grid[r][c + 1] != '<');
	}
