...
void add_antinodes(const City& map, set<Position>& antinode_positions, const char freq)
{
	...
	for (size_t i = 0; const auto& pos_a : freq_positions)
		for (const auto& pos_b : freq_positions | views::drop(++i)) {
			const int64_t diff_r = pos_b.first  - pos_a.first,
			              diff_c = pos_b.second - pos_a.second;

			int64_t antinode_r = pos_b.first,
			        antinode_c = pos_b.second;
			do {
				antinode_positions.emplace(antinode_r, antinode_c);
				antinode_r += diff_r;
				antinode_c += diff_c;
			} while (map.in_bounds(antinode_r, antinode_c));

			antinode_r = pos_a.first;
			antinode_c = pos_a.second;
			do {
				antinode_positions.emplace(antinode_r, antinode_c);
				antinode_r -= diff_r;
				antinode_c -= diff_c;
